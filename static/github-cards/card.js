...
function update_card(d, user, repo, identity, container, client_id, client_secret) {
  ...
  function linky(card, identity, container) {
  ...
  function repoCard(user, repo, identity, container) {
  ...
  if (!qs.user) {
    ...
    repoCard(qs.user, qs.repo, qs.identity, qs.container);
  } else {
    userCard(qs.user);
  }
